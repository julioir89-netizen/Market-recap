...
def calc_score(prices, macro, levels):
    ...
    above_ma50 = sum(1 for t,p in prices.items()
                     if p["price"] and levels.get(t,{}).get("ma50") and p["price"]>levels[t]["ma50"])
    score += round((above_ma50/len(prices))*30)

    spy_p   = prices.get("SPY",{}).get("price",0) or 0
    spy_ma50 = levels.get("SPY",{}).get("ma50",spy_p)
    spy_ma200= levels.get("SPY",{}).get("ma200",spy_p)
    ...
    oil_p = prices.get("OIL_CHG",0) or 0
    score += 15 if oil_p < 2 else 8 if oil_p < 4 else 3
...
def main():
    ...
    print(f"📡 Updating prices.json — {now.strftime('%I:%M %p PT')}")
